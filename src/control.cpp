...
Control::Control(PerspectiveCamera &camera, GLFWwindow *window) : camera_(camera), window_(window) {
  ...
  glfwSetCursorPosCallback(window, [](GLFWwindow *window, double x, double y) {
    ...
    auto control = static_cast<Control *>(glfwGetWindowUserPointer(window));
    control->OnMouseMove(static_cast<float>(x / width), static_cast<float>(y / height));
  });

  glfwSetScrollCallback(window, [](GLFWwindow *window, double x, double y) {
    auto control = static_cast<Control *>(glfwGetWindowUserPointer(window));
    control->OnScroll(static_cast<float>(x), static_cast<float>(y));
  });
}
...
void Control::OnKeyboardPress(float dt) {
...
void Control::OnScroll(float x, float y) {
